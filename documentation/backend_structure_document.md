...
*   Separate services for Notifications, Analytics, Integrations, Users, and Billing.
...
*   Columns: time, site_id, event_type, payload (JSONB)
*   Hypertable on `time` for efficient inserts and queries

### OLAP Schema (ClickHouse)

• Distributed tables for event aggregation, A/B test results, and long-term analytics. • Columns: site_id, timestamp, metric, value
