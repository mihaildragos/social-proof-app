...
import { VerificationMethod } from "@/app/types/sites";
import { verifyDnsTxt, getSiteVerifications, createVerificationAttempt } from "@/app/lib/sites";
import { ZodError } from "zod";

// Mock dependencies
jest.mock("@clerk/nextjs/server", () => require("../../../../__mocks__/@clerk/nextjs/server"));
jest.mock("@/utils/supabase/server", () => ({
  createClerkSupabaseClientSsr: require("../../../../__mocks__/supabase")
...
describe("Site Verification API Routes", () => {
  ...
  describe("GET /api/sites/[siteId]/verify", () => {
    ...
    it("should return verifications for a site", async () => {
      ...
      const siteVerifications = mockResponseData.verifications.filter(
        (v) => v.site_id === existingSiteId
      );
