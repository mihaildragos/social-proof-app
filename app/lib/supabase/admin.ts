...
import type { Database, Tables, TablesInsert } from "@/app/types/database";
...
const createCustomerInStripe = async (uuid: string, email: string, referral?: string) => {
  const customerData: Stripe.CustomerCreateParams = {
    metadata: { supabaseUUID: uuid, referral: referral || null },
    email: email,
  };
...
const createOrRetrieveCustomer = async ({
  ...
  const stripeIdToInsert =
    stripeCustomerId ? stripeCustomerId : await createCustomerInStripe(uuid, email, referral);
