...
import { SiteStatus, VerificationMethod, VerificationStatus, Site } from "@/app/types/sites";
import dns from "dns";
...
export const deleteSite = async (siteId: string, userId: string) => {
  ...
  const { error } = await supabase.from("sites").delete().eq("id", siteId).eq("owner_id", userId);
...
export const verifyDnsTxt = async (siteId: string, userId: string) => {
  ...
  try {
    ...
    const verificationFound = records
      .flat()
      .some((record) => record.includes(site.verification_token));
