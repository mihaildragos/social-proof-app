...
import { createVerificationAttempt, verifyDnsTxt, getSiteVerifications } from "@/app/lib/sites";
import { createVerificationSchema, VerificationMethod } from "@/app/types/sites";
...
export async function POST(req: Request, { params }: { params: { siteId: string } }) {
  try {
    ...
    const verification = await createVerificationAttempt(
      siteId,
    ...
    return NextResponse.json(
      {
        error: {
          message: "Failed to create verification attempt",
          details: (error as Error).message,
        },
      },
      { status: 500 }
