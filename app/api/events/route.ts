  ...
  try {
    ...
    try {
      ...
      if (event.event_type === 'impression') {
        await redis.hincrby(`notification:status:${event.data.notification_id}`, 'delivered_count', 1);
        
        // Set first delivered timestamp if this is the first delivery
        const deliveredCount = await redis.hget(`notification:status:${event.data.notification_id}`, 'delivered_count');
        if (deliveredCount === '1') {
          await redis.hset(
        ...
        await redis.hincrby(`notification:status:${event.data.notification_id}`, 'clicked_count', 1);
        
        // Set first click timestamp if this is the first click
        const clickedCount = await redis.hget(`notification:status:${event.data.notification_id}`, 'clicked_count');
        if (clickedCount === '1') {
          await redis.hset(
            `notification:status:${event.data.notification_id}`,
            'first_clicked_at',
