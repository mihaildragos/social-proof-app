    ...
    try {
      ...
      const userData = await prisma.user.findUnique({
        ...
        select: {
          ...
          email: true,
          fullName: true,
          // firstName: true, // Field not in database schema
          // lastName: true, // Field not in database schema
          // organizationId: true, // Field not in database schema
      ...
      // Always get from organization membership since user table doesn't have these fields
      {
      ...
      const fullName = userData.fullName || `User ${userId.substring(0, 8)}`;
