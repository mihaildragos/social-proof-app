 ...
 */
export async function generateToken(payload: Omit<JWTClaims, 'iat' | 'exp' | 'nbf' | 'iss' | 'aud' | 'jti'>): Promise<string> {
...
export async function blacklistToken(jti: string, expiresAt: number): Promise<void> {
  try {
    await db.query(
      `INSERT INTO token_blacklist (token_id, expires_at) VALUES ($1, $2)`,
...
async function checkTokenBlacklist(jti: string): Promise<boolean> {
  try {
    const result = await db.query(
      `SELECT 1 FROM token_blacklist WHERE token_id = $1`,
      [jti]
    );
