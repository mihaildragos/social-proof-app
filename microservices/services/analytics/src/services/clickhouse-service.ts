  ...
  async getDeviceAnalytics(
    ...
    return {
      ...
      browsers: Array.from(browsers.entries())
        ...
        .sort((a, b) => b.users - a.users),
      operating_systems: Array.from(operatingSystems.entries())
        .map(([os, users]) => ({
          operating_system: os,
          users,
        }))
    ...
    let query = `
      WITH step_events AS (
        SELECT 
          ...
          ${steps
            .map(
              (step, index) => `if(event_name = '${step}', ${index + 1}, 0) as step_${index + 1}`
            )
            .join(", ")}
        FROM analytics_events
        WHERE organization_id = {organization_id:String}
          ...
          .map(
            (_, index) =>
              `countIf(step_${index + 1}_time IS NOT NULL ${
                index > 0 ?
                  `AND step_${index + 1}_time >= step_${index}_time AND step_${index + 1}_time <= step_${index}_time + INTERVAL ${windowHours} HOUR`
                : ""
