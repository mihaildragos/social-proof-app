...
export class TemplateRenderer {
  ...
  private extractDynamicValues(
    ...
    if (template.content && template.content.variables) {
      const variables = template.content.variables as Record<
        string,
  ...
  private registerHelpers(): void {
    ...
    Handlebars.registerHelper(
      "formatCurrency",
      function (value: string | number, currency: string) {
    ...
    Handlebars.registerHelper(
      "ifCond",
      function (this: any, v1: any, operator: string, v2: any, options: HelperOptions) {
        switch (operator) {
          case "==":
