  ...
  public async handle(event: OrderEvent): Promise<void> {
    try {
      ...
      if (templates.length === 0) {
        this.logger.info('No templates found for event', { eventType: event.type, siteId: event.siteId });
        return;
      }
      ...
      for (const template of templates) {
        ...
        if (!shouldDisplay) {
          this.logger.info('Notification filtered out by targeting rules', { notificationId: notification.id });
          continue;
        }
        ...
        this.logger.info('Successfully processed notification', { notificationId: notification.id });
      }
    } catch (error) {
