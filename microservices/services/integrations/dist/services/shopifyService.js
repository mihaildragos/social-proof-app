...
const SHOPIFY_ADMIN_API_URL = (shopDomain) =>
...
export class ShopifyService {
  ...
  static async registerWebhooks(
    shopifyStore,
    webhookUrl,
    topics = [SHOPIFY_WEBHOOK_TOPICS.ORDERS_CREATE]
  ) {
   ...
   */
  static verifyWebhookSignature(hmac, body, shopifySecret) {
   ...
   */
  static async processOrderWebhook(shopDomain, orderData) {
    try {
   ...
   */
  static transformOrderData(orderData, shopifyStore) {
    ...
    const totalItems =
      orderData.line_items ?
        orderData.line_items.reduce((sum, item) => sum + (item.quantity || 0), 0)
      : 0;
   ...
   * @param shopDomain Shopify store domain
   */
  static async processAppUninstalledWebhook(shopDomain) {
    try {
      // Find the Shopify store
      const shopifyStore = await ShopifyIntegration.findByDomain(shopDomain);
      if (!shopifyStore) {
   ...
   */
  static async createScriptTag(shopifyStore, scriptUrl) {
    if (!shopifyStore.access_token) {
      throw new Error("Shopify access token not available");
    }
    try {
